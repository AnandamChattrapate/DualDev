...
function MyConnectionPill() {
  ...
  return (
    <div style={{
      ...
      <span style={{
        fontFamily: "var(--mono)", fontSize: 12, letterSpacing: "0.14em", color,
      }}>
...
export default function Match() {
  ...
  useEffect(() => {
    socket.on("verdict", ({ userId: sid, verdict, results, testsPassed }) => {
      if (sid !== userId) return
      setMyVerdict({ verdict, results, testsPassed })
      setSubmitting(false)
      incrementSubmission()
      socket.emit("tc_update", { matchId, testsPassed, totalTests: results?.length || 0 })
    })
    ...
    socket.on("match_result",       ({ winnerId, aiReview })               => { setWinner(winnerId); if (aiReview) setAIReview(aiReview); setMatchEnded(true); navigate(`/result/${matchId}`) })
  ...
  }, [matchId, userId, myCode, myLanguage, matchEnded])
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button className="btn" onClick={runCode} disabled={isRunning} aria-label="Run sample tests">
          <IconPlay s={12} /> {isRunning ? "Running…" : "Run"}
        </button>
        <button className="btn btn-accent" onClick={submitCode} disabled={isSubmitting} aria-label="Submit solution">
