...
export default function Result() {
  ...
  // demo values – replace with real data from store when available
  const myRatingBefore = 1420;
  const myRatingAfter = iWon ? 1448 : 1398;
  const ratingDiff = myRatingAfter - myRatingBefore;

  const myTime = "1m 42s";
  const oppTime = "2m 18s";

  // fade-in animation
  const [show, setShow] = useState(false);
  useEffect(() => {
    // Clean up match data when the user leaves the result page
    return () => {
      ...
      >
        ...
        <div className="flex items-center justify-between mb-10">
          <h1 className="font-claude text-[42px] font-bold tracking-[-2px]">
            ...
            <span className="text-[#F4B183]">JUDGE</span>
          </h1>
            ...
            <div className="flex flex-col items-center justify-center pt-12">
              ...
              <div className="w-full mt-2 flex gap-1">
                <div
                  ...
                  style={{
                    width: `${(myTestsPassed / oppTotalTests) * 100}%`,
                  }}
                />
                <div
                  className="h-1 rounded-full bg-[#FF7A00] transition-all"
                  style={{
                    width: `${(oppTestsPassed / oppTotalTests) * 100}%`,
                  }}
                />
              </div>
            </div>
              ...
              <div className="text-right">
                ...
                <div
                  className={`mt-2 text-lg font-semibold ${
                    ratingDiff >= 0 ? "text-[#00FF85]" : "text-[#FF5A5A]"
                  }`}
                >
                  {ratingDiff >= 0 ? "+" : ""}
...
function PlayerCard({ player, label, color, testsPassed, totalTests, time, language }) {
  const pct = totalTests ? (testsPassed / totalTests) * 100 : 0;
  const barColor = color + "80"; // semi-transparent
